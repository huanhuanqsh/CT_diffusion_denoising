...
class DDPM(BaseModel):
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                self.SR = self.netG.module.super_resolution(
                    self.data['SR'], *args, continous=continous, ddim=ddim)
            else:
    ...
    def sample(self, *args, batch_size=1, continous=False, ddim=False):
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                self.SR = self.netG.module.sample(*args, batch_size=batch_size, continous=continous, ddim=ddim)
            else:
                self.SR = self.netG.sample(*args, batch_size=batch_size, continous=continous, ddim=ddim)
        self.netG.train()
    ...
    def inversion(self, noisy, sr, *args, batch_size=1, ddim=False, lambda1=0.0075, a=1.5, b=-0.01, c=0.3, resume=3, mode=0, continous=False):
        ...
        with torch.no_grad():
            if isinstance(self.netG, nn.DataParallel):
                self.SR = self.netG.module.inversion(noisy, sr, *args, batch_size=batch_size, ddim=ddim, 
                                                     lambda1=lambda1, a=a, b=b, c=c, resume=resume, mode=mode, continous=continous)
            else:
                self.SR = self.netG.inversion(noisy, sr, *args, batch_size=batch_size, ddim=ddim, 
                                              lambda1=lambda1, a=a, b=b, c=c, resume=resume, mode=mode, continous=continous)
        self.netG.train()
